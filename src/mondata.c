...
const char *in_str;
{
    ...
    {
        static const struct alt_spl names[] = {//²»ÓÃ·­Òë
            /* Alternate spellings */
            ...
            if (m_i_len == slen) {
                ...
                break; /* exact match */
                       ...
                       && (str[m_i_len] == ' '
                           ...
                           || !strcmpi(&str[m_i_len], "'s")
                           || !cnstrcmp(&str[m_i_len], "Ð¡")  //Ð¡µñÏñ
                           || !cnstrcmp(&str[m_i_len], "µñ")  //µñÏñ
                           || !cnstrcmp(&str[m_i_len], "Ê¬")  //Ê¬Ìå
                           || !cnstrcmp(&str[m_i_len], "ÍÅ")  //²¼¶¡ÍÅ
                           || !cnstrcmp(&str[m_i_len], "¹Þ"))) {  //¹ÞÍ·
                mntmp = i;
                len = m_i_len;
            }
        }
...
int *mndx_p;
{
    ...
    static NEARDATA const struct alt_spl truematch[] = {
        ...
        { "¶ñÄ§", -S_DEMON }, /* hits "imp or minor demon" */
        /* matches specific monster (overly restrictive) */
        { "Ä§¹í", -S_DEMON }, /* always "horned devil" */
        /* some plausible guesses which need help */
        { "³æ", -S_XAN },  /* would match bugbear... */
        { "Óã", -S_EEL }, /* wouldn't match anything */
        /* end of list */
        { 0, NON_PM }
    };
    const char *p, *x;
    ...
    if (!in_str || !in_str[0]) {
        ...
        /* multiple characters */
        if (!strcmpi(in_str, "long")) /* ²»ÐèÒª¸Ä not enough to match "long worm" */
...
static const char *levitate[4] = { "Æ®", "Æ®", "Ò¡»Î", "Ò¡»Î" };
static const char *flys[4] = { "·É", "·É", "Æ®¶¯", "Æ®¶¯" };
static const char *flyl[4] = { "·É", "·É", "õçõÇ", "õçõÇ" };
static const char *slither[4] = { "»¬", "»¬", "õÔõÄ", "õÔõÄ" };
static const char *ooze[4] = { "Éø", "Éø", "²ü¶¶", "²ü¶¶" };
static const char *immobile[4] = { "°Ú¶¯", "°Ú¶¯", "Õð¶¯", "Õð¶¯" };
static const char *crawl[4] = { "ÅÀ", "ÅÀ", "õÔõÄ", "õÔõÄ" };
...
struct attack *mattk;
{
    ...
    case PM_ENERGY_VORTEX:
        ...
        break;
    default:
        what = (mattk->aatyp == AT_HUGS) ? "±»¿¾" : "×Å»ðÁË";
        break;
    }
    return what;
}
